/**
 * Deploy VeilConsent to a Midnight network (local devnet by default;
...
async function createProviders(walletCtx: WalletContext) {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<'createRequest'>(zkConfigPath);
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy VeilConsent to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // empty for VeilConsent's no-arg constructor. (Statically-typed contracts can omit
